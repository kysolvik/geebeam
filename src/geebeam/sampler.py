...
def _get_crs_scale(
    ...
    transform = ee.Projection(crs=crs).atScale(scale_m).getInfo()
    return transform[0]
...
def sample_region_random(
    ...
    sampled_points = roi.sample_points(n_sample, rng=rng).geometry.explode()
...
def sample_region_grid(
    ...
    x_all, y_all = np.meshgrid(x_locs, y_locs)

    points_gdf = gpd.GeoDataFrame(geometry=gpd.points_from_xy(x_all, y_all))
...
def _assign_splits_pandas(df, split_dict, random_seed=0, shuffle=True):
    ...
    cur_index = 0
    for split_name, split_count in split_dict.keys():
        df.loc[df.index[cur_index:cur_index+split_count], 'split'] = split_name
...
def split_sets(
    ...
    if len(split_names) <= 0:
        if split_ratios:
            if np.sum(split_ratios) > 1:
                raise ValueError('Split ratios sum to more than 1.')
                ...
                split_counts = np.rint(np.array(split_ratios) * points_gdf.shape[0])
            ...
            if np.sum(split_counts) > total_points:
                raise ValueError('Split counts sum to more than 1.')
            elif len(split_ratios) != len(split_names):
    ...
    split_dict = zip(split_names, split_counts)
